...
export default function PromotionInput() {
  ...
  const [documentNumberToupdate, setdocumentNumberToupdate] = useState(''); 
  ...
  const { rowData: promoRowData,rowData2: promoRowData2, fetchPromotionDetails, promotion, customer, promotionDetails, proposals, isLoading } = usePromo();
...
useEffect(() => {
  if (rowData.length > 0) {
    const newSelectedColumns = Calbasegrowth.reduce((selectedIndexes, month, index) => {
      if (index > 1) { // ข้าม `Productname` และ `Packsize`
        const monthKey = `${month.toLowerCase()}Selected`;
        const isMonthSelected = rowData.some(row => row[monthKey] === true);
        if (isMonthSelected) {
...
useEffect(() => {
  ...
  if (!Array.isArray(rowData2) || rowData2.length === 0) {
    console.warn('rowData2 is empty or not an array');
    return;
  }

  const newSelectedColumns = Calbasegrowth2.reduce((selectedIndexes, month, index) => {
    if (index > 1) { // ข้าม Productname และ Packsize
      // ตัดเลข 2 ออกจากชื่อเดือนก่อนสร้าง monthKey
      const monthWithoutNumber = month.replace('2', '');
      const monthKey = `${monthWithoutNumber.toLowerCase()}Selected2`;
      
      // ตรวจสอบค่าใน rowData2
      const hasValue = rowData2.some(row => {
        return row[monthKey] === true;
      });
  ...
  // อัพเดทค่า selected columns
  setSelectedColumns2(newSelectedColumns);
  
  // แสดงผลลัพธ์สุดท้าย
  // console.log('Final selected columns:', newSelectedColumns);
  // console.log('Complete rowData2:', rowData2);
...
const handleColumnCheckboxChange2 = (index) => {
  // ตัดเลข 2 ออกจากชื่อเดือนก่อนสร้าง fieldName
  const monthWithoutNumber = Calbasegrowth2[index]?.replace('2', '');
  const fieldName = monthWithoutNumber?.toLowerCase() + 'Selected2';
  ...
  setSelectedColumns2((prevSelectedColumns) => {
    ...
    // อัปเดตค่าใน rowData2
    setRowData2((prevRowData) =>
      prevRowData.map((row) => ({
        ...row,
        [fieldName]: newSelectedColumns.includes(index),
      }))
    );
    return newSelectedColumns;
  });
...
useEffect(() => {
  if (promoRowData) {
    const formattedRowData = promoRowData.map(row => ({
      productCode: row.productCode,
      productNameEng: row.product.productNameEng, // ดึงข้อมูลจาก product
      price: row.rsp, 
      packsize: row.packSize,
      GP: row.gp, 
      Based: row.base,
      Growth: row.growth,
      estimateSales: row.estimateSales,
      wspPerCarton: row.wspPerCarton,
      totalWsp: row.totalWsp,
      compensatePcs: row.compensatePerPcs,
      pencencompensate: row.pencencompensate,
      avgPercent: row.avgPercent,
      discount: row.compensateDiscount,
      baseLine: row.baseLine,
      growthcal: row.growthcal,
      totalCTN: row.totalCTN,
      totalPcs: row.totalPcs,
      offTake: row.offTake,
      totalBased: row.totalBased,
      ...
      nov: row.nov,
      dec: row.dec,
      janSelected: row.janSelected,
      febSelected: row.febSelected,
      marSelected: row.marSelected,
      aprSelected: row.aprSelected,
      maySelected: row.maySelected,
      junSelected: row.junSelected,
      julSelected: row.julSelected,
      augSelected: row.augSelected,
      sepSelected: row.sepSelected,
      octSelected: row.octSelected,
      novSelected: row.novSelected,
      decSelected: row.decSelected,
      janCal: row.janCal,
      febCal: row.febCal,
      marCal: row.marCal,
      aprCal: row.aprCal,
    ...
    // อัปเดต rowData และ initialRowData
    setRowData(formattedRowData);
    setInitialRowData(JSON.parse(JSON.stringify(formattedRowData))); // สำเนาแบบ deep copy
  }

  if (promotion) {
    if (promotion.documentNumber) {
      setdocumentNumberToupdate(promotion.documentNumber);
    }
    ...
    if (promotion.store_applied) {
      setBranchCount(promotion.store_applied);
    }
    if (promotion.storeSize) {
      setStoreSize(promotion.storeSize);
      if (promotion.storeSize === "1.2M") {
        setStoreSizeMapped(200);
      } else if (promotion.storeSize === "1M") {
        setStoreSizeMapped(100);
      } else if (promotion.storeSize === "80M") {
        setStoreSizeMapped(95);
      }
    }
    if (promotion.accountOpeningFee) {
      setaccountOpeningFee(promotion.accountOpeningFee);
    }
    if (promotion.listingFee) {
      setListingFee(promotion.listingFee);
    }
    if (promotion.systemCost) {
      setsystemCost(promotion.systemCost);
    }
    if (promotion.placementFee) {
      setplacementFee(promotion.placementFee);
    }
    if (promotion.electricityFee) {
      setelectricityFee(promotion.electricityFee);
    }
    if (promotion.focfreeicecreamFee) {
      setfocfreeicecreamFee(promotion.focfreeicecreamFee);
    }
    if (promotion.focsupportFee) {
      setFocsupportFee(promotion.focsupportFee);
    }
    if (promotion.otherFees) {
      setOtherCosts(promotion.otherFees);
    }
    if (promotion.detailpromotion) {
      setDetailPromotion(promotion.detailpromotion);
    }
    if (promotion.remark) {
      setRemark(promotion.remark);
    }
    if (promotion.createdAt) {
      setCurrentDate(new Date(promotion.createdAt).toISOString().split('T')[0]);
    }
    ...
    if (promotion.baseGrowthstartDate) {
      setBaseGrowthStartDate(new Date(promotion.baseGrowthstartDate).toISOString().split('T')[0]);
    }
    if (promotion.baseGrowthendDate) {
      setBaseGrowthEndDate(new Date(promotion.baseGrowthendDate).toISOString().split('T')[0]);
    }
  }
  ...
  if (customer) {
    ...
    if (customer.CustomerCode) {
      setSelectedCompany(customer.CustomerCode);
    }
  }
}, [promoRowData, promotion, customer]);
...
useEffect(() => {
  if (promoRowData2) {
    const formattedRowData = promoRowData2.map(row => ({
      productCode: row.productCode,
      productNameEng: row.product.productNameEng,
      packsize: row.packSize,
      jan2: row.jan2,
      feb2: row.feb2,
      mar2: row.mar2,
      ...
      novCal2: row.novCal2,
      decCal2: row.decCal2,
      janSelected2: row.janSelected2,
      febSelected2: row.febSelected2,
      marSelected2: row.marSelected2,
      aprSelected2: row.aprSelected2,
      maySelected2: row.maySelected2,
      junSelected2: row.junSelected2,
      julSelected2: row.julSelected2,
      augSelected2: row.augSelected2,
      sepSelected2: row.sepSelected2,
      octSelected2: row.octSelected2,
      novSelected2: row.novSelected2,
      decSelected2: row.decSelected2,
    }));
    setRowData2(formattedRowData);
    setInitialRowData2(JSON.parse(JSON.stringify(formattedRowData))); // สำเนาแบบ deep copy
...
////////////////////////////////////////////////////////////////
  ...
  const productOptions = products.map((product) => ({
    ...
    label: `${product.productCode} - ${product.productNameEng}`,
  }));
...
const handleBaseGrowthCal = async () => {
  ...
  const handleSubmitToAddPromotion = async () => {
    ...
    try {
        ...
        const response = await axios.patch(
            "/PromotionProposal/api/promotion/updatepromotion",
            dataToSubmit
        );
  ...
  const [columnDefs] = useState([
    ...
    { headerName: "Packsize", field: "packsize", editable: false, valueFormatter: numberFormatter },
    {
      ...
      valueSetter: handleGPChange,
      valueFormatter: numberFormatter,
    },
    { headerName: "Based", field: "Based", editable: true, cellStyle: { backgroundColor: '#A7E7EF' }, valueFormatter: numberFormatter },
    { headerName: "Growth", field: "Growth", editable: true, cellStyle: { backgroundColor: '#A7E7EF' }, valueFormatter: numberFormatter },
    {
    ...
    {
      headerName: "COMPENSATE / Pcs.",
      field: "compensatePcs",
      editable: true,
      cellStyle: { backgroundColor: '#A7E7EF' },
      valueSetter: handleCompensatePcsChange,
      valueFormatter: numberFormatter,
    },
    {
      headerName: "PencenCOMPENSATE",
      field: "pencencompensate",
      editable: true,
      cellStyle: { backgroundColor: '#A7E7EF' },
      valueSetter: handlePencenCompensateChange,
      valueFormatter: numberFormatter,
    },
    { headerName: "Avg. %", field: "avgPercent", editable: false, valueFormatter: numberFormatter },
    {
      ...
      field: "discount",
      editable: false,
      valueFormatter: (params) => `${numberFormatter(params)}%`, // แสดงผลเป็น %
    },
    {
      field: 'actions',
      headerName: '',
...
<CircularProgress color="inherit" />
      ...
      <section className="border-b-2 pb-4 mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" readOnly className="mt-1 p-2 block w-full border rounded-md focus:outline-none cursor-not-allowed"  value={documentNumberToupdate|| ''} />
          </div>
          <div>
            ...
            <input
              ...
              className="mt-1 p-2 block w-full border rounded-md focus:outline-none  cursor-not-allowed"
            />
          </div>
          <div>
  ...
  <input 
    type="text" 
    value={promotionTitle|| ''} // ใช้ promotion.promotionTitle ถ้ามีค่า หากไม่มีใช้ ''
  ...
  />
          ...
          <div>
            ...
            <input type="text" 
            value={promotionType|| ''} // ใช้ promotion.promotionTitle ถ้ามีค่า หากไม่มีใช้ ''
      ...
      <div className="flex flex-wrap space-x-0 md:space-x-4">
        <div className="w-full md:w-auto mb-2 md:mb-0">
          ...
          <input
            ...
            min={new Date().toISOString().split('T')[0]} // จำกัดการเลือกวันที่เป็นวันนี้หรือหลังจากนี้
          />
        </div>
        <div className="w-full md:w-auto">
          ...
          <input
            ...
            min={startDate} // จำกัดไม่ให้เลือกวันที่น้อยกว่า startDate
            disabled={!startDate} // ปิดการใช้งานจนกว่าจะเลือก startDate
          />
        </div>
      </div>
    </div>
...
<section className="border-b-2 pb-2 mb-6">
...
<div className="mb-2">
  ...
  <div className="text-xs text-red-500 bg-red-50 p-2 rounded-md border border-red-200">
    <span className="font-medium">หมายเหตุ: Estimate Growth Sale</span> (หน่วย/ชิ้น) ยอดขายที่คาดว่าจะเกิดขึ้นในช่วงระยะเวลาจัดโปรโมชั่น : จำนวนชิ้น ต่อสาขา ต่อวัน (โดยประเมินจากเป้าหมายการเติบโตของยอดขายที่เกิดจากโปรโมชั่น เช่น ส่วนลด หรือกิจกรรมส่งเสริมการขาย อาจอ้างอิงจากประวัติการจัดโปรโมชั่นก่อนหน้า หรือ Benchmark ของสินค้าคล้ายกันในช่วงเวลาเดียวกัน)
      ...
      {/* Ag-Grid Table */}
      <section className="ag-theme-alpine" style={{ height: 400, width: '100%' }}>
        <AgGridReact
          rowData={rowData}
        ...
        />
          ...
          <div className=' flex justify-end items-center align-middle'>
            <button onClick={handleCalculatePromo} className="mt-4 p-2 bg-blue-500 text-white rounded-md ">
              คำนวณ
            </button>
          </div>
